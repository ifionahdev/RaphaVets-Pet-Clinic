...
function createOCRChecker(config = {}) {
  const {
    languages = ["english"],
    tesseractLangs = ["eng"],
    detectLeetspeak = true,
    normalizeUnicode: normalizeUnicode2 = true,
    minConfidence = 30,
    filterConfig = {}
  } = config;
  const filter = new Filter({
...
async function checkImageForProfanity(image, config = {}) {
...
async function batchCheckImages(images, config = {}) {
  const checker = createOCRChecker(config);
  try {
...
function createAudioPipeline(config) {
  ...
  return {
     ...
     */
    censorTranscript(text, replacement = "***") {
...
function createGoogleSTTTranscriber(config) {
  const {
    apiKey,
    languageCode = "en-US",
...
function createRealtimeChecker(config) {
  const {
    transcriber,
    onProfanityDetected,
    languages = ["english"],
...
exports.LANGUAGE_TO_TESSERACT = LANGUAGE_TO_TESSERACT;
exports.batchCheckImages = batchCheckImages;
exports.checkImageForProfanity = checkImageForProfanity;
exports.createAudioPipeline = createAudioPipeline;
exports.createGoogleSTTTranscriber = createGoogleSTTTranscriber;
exports.createOCRChecker = createOCRChecker;
exports.createRealtimeChecker = createRealtimeChecker;
exports.createWhisperTranscriber = createWhisperTranscriber;
exports.languagesToTesseract = languagesToTesseract;
